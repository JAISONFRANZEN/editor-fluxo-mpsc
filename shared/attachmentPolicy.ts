...
export const MAX_COMMENT_ATTACHMENT_TOTAL_BYTES = 10 * 1024 * 1024;
...
export function validateCommentAttachment(input: { filename: string; mimeType: string; size: number }) {
  ...
  if (input.mimeType === "application/octet-stream" && !/\.bpmn$/i.test(input.filename)) return "Arquivos binários genéricos são aceitos somente para diagramas .bpmn.";
  return null;
}
